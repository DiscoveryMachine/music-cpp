...
// Validate
void ControlInitiatedMessage::Validate() const {
  if (type_ != MusicMessageType::kControlInitiated) {
    LogError(
        "ControlInitiatedMessage::Validate - type_ must be kControlInitiated");
  }
  if (command_identifier_ !=
      MusicMessageTypeHelper::GetInfoForType(type_).action_type) {
    LogError(
        "ControlInitiatedMessage::Validate - command_identifier_ does not "
        "match type_: " +
...
// JSON serialization
void to_json(nlohmann::json &j, const ControlInitiatedMessage &message) {
  // fill out super class json
  to_json(j, static_cast<const MusicCommandRequestMessage &>(message));
  // No additional fields to serialize
}

void from_json(const nlohmann::json &j, ControlInitiatedMessage &message) {
