...
namespace music {
// Validate
void ControlRelinquishedMessage::Validate() const {
  if (type_ != MusicMessageType::kControlRelinquished) {
    LogError(
        "ControlRelinquishedMessage::Validate - type_ must be "
...
// JSON Serialization
void to_json(nlohmann::json &j, const ControlRelinquishedMessage &message) {
  to_json(j, static_cast<const MusicRequestMessage &>(message));
  // No additional fields to serialize
}
void from_json(const nlohmann::json &j, ControlRelinquishedMessage &message) {
