...
namespace music {
// Validate
void ControlRegainedMessage::Validate() const {
  if (type_ != MusicMessageType::kControlRegained) {
    LogError(
        "ControlRegainedMessage::Validate - type_ must be kControlRegained");
  }
  MusicRequestMessage::Validate();
}

// JSON Serialization
void to_json(nlohmann::json &j, const ControlRegainedMessage &message) {
  to_json(j, static_cast<const MusicRequestMessage &>(message));
  // No additional fields to serialize
}
void from_json(const nlohmann::json &j, ControlRegainedMessage &message) {
