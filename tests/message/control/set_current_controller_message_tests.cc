...
TEST_GROUP(SetCurrentControllerMessageTest){
    // SETUP
};

TEST(SetCurrentControllerMessageTest, Validate) {
  auto valid_header =
  ...
  auto valid_current_controller_id =
  ...
  SetCurrentControllerMessage message =
      SetCurrentControllerMessageBuilder()
          ...
          .SetRequestId(10)
          .SetCurrentControllerId(valid_current_controller_id)
          .Build();
  ...
  // Set invalid current controller id
  message.SetCurrentControllerId(EntityId());
  CHECK_THROWS(std::exception, message.Validate());
  message.SetCurrentControllerId(valid_current_controller_id);
}

TEST(SetCurrentControllerMessageTest, ToJson) {
  SetCurrentControllerMessage message =
      SetCurrentControllerMessageBuilder()
  ...
  // Check current controller ID
  LONGS_EQUAL(7, json["currentControllerID"]["siteID"].get<int>());
  LONGS_EQUAL(8, json["currentControllerID"]["appID"].get<int>());
  LONGS_EQUAL(9, json["currentControllerID"]["entityID"].get<int>());
}

TEST(SetCurrentControllerMessageTest, FromJson) {
  ...
  SetCurrentControllerMessage message =
      SetCurrentControllerMessage::FromJson(json);
  ...
  // Check current controller ID
  LONGS_EQUAL(7, message.GetCurrentControllerId().GetSite());
  LONGS_EQUAL(8, message.GetCurrentControllerId().GetApplication());
  LONGS_EQUAL(9, message.GetCurrentControllerId().GetEntityNumber());
}
}  // namespace music::test